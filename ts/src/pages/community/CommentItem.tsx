import { CommentItemData } from "#types/board";
...
function CommentItem({ item, _id, refetch }: CommentItemProps) {
        ...
        <div className="flex justify-between items-center mb-2">
          {reply.user?.profile ? (
            reply.user.profile.path ? (
              ...
              <img
                className="w-8 mr-2 rounded-full"
                src={`http://api.fesp.shop/${reply.user.profile}`}
                alt={`${reply.user.name} 프로필 이미지`}
              />
            )
          ) : (
